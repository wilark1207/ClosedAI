...
def is_modif_or_description(prompt):
    result = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "user", "content": "Is this asking to read the calendar (type1) or is it asking to write on the calendar (type2) or if it is not relevant to calendars or time (type3), answer in this format: This is: Type(1 or 2 or 3 would go here) - " + prompt}
...
def get_results(prompt, calendar):
    ...
    if (type == DESCRIPTIVE_PROMPT):
        result = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Your calendar is this json object - " + json.dumps(calendar)},
        ...
        result = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "The format of the calendar is this json object - " + json.dumps(calendar)},
